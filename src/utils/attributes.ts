...
export const renderAttribute = (card: FlowerCard, attr: DisplayedAttribute) => {
  ...
  return html`
    ...
    >
      ...
      ${card.config.display_type === DisplayType.Compact
        ? ""
...
export const renderAttributeChunks = (
  ...
  const chunkedDisplayed = getChunkedDisplayed(
    displayed,
    card.config.display_type === DisplayType.Compact ? 1 : 2,
  );
